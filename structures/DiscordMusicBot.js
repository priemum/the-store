...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    this.Manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(`Lavalink: Node ${node.options.identifier} connesso`)
      )
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: Node ${node.options.identifier}ha avuto un errore: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        this.SongsPlayed++;
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Ora in riproduzione ♪`, this.botconfig.IconURL)
          .setThumbnail(player.queue.current.displayThumbnail())
          .setDescription(`[${track.title}](${track.uri})`)
          .addField("Richiesta da", `${track.requester}`, true)
          .addField(
            "Durata",
      ...
      .on("queueEnd", (player) => {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("La coda è finita", this.botconfig.IconURL)
          .setColor(this.botconfig.EmbedColor)
          .setTimestamp();
      ...
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "Impossibile caricare il comando: " +
                file.split(".")[0] +
                ", Motivo: il file non ha esecuzione/nome/descrizione"
            );
          this.commands.set(file.split(".")[0].toLowerCase(), cmd);
          this.log("Comando Caricato: " + file.split(".")[0]);
        });
    });
  }
    ...
    fs.readdir(EventsDir, (err, files) => {
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          this.logger.log("Evento Caricato: " + file.split(".")[0]);
        });
    });
  }
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Se ritieni che questo sia un bug, segnalalo al server di supporto!"
      );
  ...
  build() {
    ...
    if(this.botconfig.ExpressServer){
      this.http.listen(process.env.PORT || this.botconfig.Port, () => this.log("Il server Web è stato avviato"));
    }
  }
