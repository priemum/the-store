...
module.exports = {
  name: "lyrics",
  description: "Mostra il testo della canzone cercata",
  usage: "[Song Name]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | **Non sta suonando niente in questo momento...**");
    ...
    if (!lyrics) return client.sendTime(message.channel, `**Nessun testo trovato per-** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
    let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page

    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`Testo di: ${SongTitle}`, client.botconfig.IconURL)
  ...
  SlashCommand: {
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "Inserisci il nome di una canzone da cercare",
        required: false,
      },
    ],
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "❌ | **Non sta suonando niente in questo momento...**");
      ...
      if (lyrics.length === 0)
        return client.sendTime(interaction, `**Nessun testo trovato per -** \`${SongTitle}\``);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page
