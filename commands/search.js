...
module.exports = {
  name: "search",
  description: "Mostra un risultato di brani in base alla query di ricerca",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **Devi essere in un canale vocale per riprodurre qualcosa!**"
      );
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Utilizzo - **\`${GuildDB.prefix}search [query]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Nessuna corrispondenza trovata per " + SearchString
      );
    else {
      ...
      let Pages = songs.map((songz) => {
        let MappedSongs = songz.map(
          (s) =>
            `\`${s.index + 1}.\` [${s.title}](${
              s.uri
            }) \nDurata: \`${prettyMilliseconds(s.duration, {
              colonNotation: true,
            })}\``
        );

        let em = new MessageEmbed()
          .setAuthor("Risultati della ricerca di " + SearchString, client.botconfig.IconURL)
      ...
      let msg = await message.channel.send(
        "**Digita il numero della canzone che vuoi suonare! Scade tra `30 secondi`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Hai impiegato troppo tempo per rispondere. Esegui di nuovo il comando se vuoi riprodurre qualcosa!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(message.channel, "Si prega di inviare il numero ID del brano corretto");
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song) return client.sendTime(message.channel, "Nessun brano trovato per l'ID specificato");
      ...
      SongAddedEmbed.addField("Autore", `${Song.author}`, true);
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Inserisci il nome del brano o l'URL che vuoi cercare",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (
        ...
        return client.sendTime(
          interaction,
          ":x: | **Devi essere nel mio stesso canale vocale per usare questo comando!**"
        );
      let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
      if (!CheckNode || !CheckNode.connected) {
        return client.sendTime(
          interaction,
          "❌ | **Lavalink node non connesso**"
        );
      }
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "NO_MATCHES":
            if (!player.queue.current) player.destroy();
            return client.sendTime(interaction, ":x: | **Nessun risultato trovato**");
          case "TRACK_LOADED":
            player.queue.add(TrackUtils.build(Searched.tracks[0], member.user));
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Aggiunto alla coda:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
          ...
          case "PLAYLIST_LOADED":
            ...
            if (
              ...
              player.play();
            return client.sendTime(
              interaction, `**Playlist aggiunta alla coda**: \n**${Searched.playlist.name}** \nIn coda: **${Searched.playlistInfo.length} canzoni**`
            );
        }
      } else {
        try {
          ...
          return client.sendTime(
            interaction, `:x: | **Si è verificato un errore durante la ricerca:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "TRACK_LOADED":
            ...
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Aggiunto alla coda:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
            ...
            if (
              ...
              player.play();
            return client.sendTime(
              interaction, `**Playlist aggiunta alla coda**: \n**${res.playlist.name}** \In coda: **${res.playlistInfo.length} canzoni**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Digita il numero della canzone che vuoi riprodurre!**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(`cerca risultati per ${search}`, client.botconfig.IconURL);
            ...
            if (first.toLowerCase() === "cancel") {
              if (!player.queue.current) player.destroy();
              return awaitchannel.send("Ricerca annullata.");
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Il numero che hai fornito era maggiore o minore del totale della ricerca. Utilizzo - \`(1-${max})\``
              );
