    ...
    switch (stateChange.type) {
        case "JOIN":
            if (stateChange.members.size === 1 && player.paused) {
                let emb = new MessageEmbed()
                    .setAuthor(`Ripresa della coda in pausa`, client.botconfig.IconURL)
                    .setColor("RANDOM")
                    .setDescription(`Riprendo la riproduzione perché tutti voi mi avete lasciato con la musica per suonare da solo`);
        ...
        case "LEAVE":
            if (stateChange.members.size === 0 && !player.paused && player.playing) {
                ...
                let emb = new MessageEmbed()
                    .setAuthor(`In pausa!`, client.botconfig.IconURL)
                    .setColor(client.botconfig.EmbedColor)
                    .setDescription(`Il bot è stato messo in pausa perché tutti se ne sono andati`);
                await client.channels.cache.get(player.textChannel).send(emb);
            }
            break;
    }
